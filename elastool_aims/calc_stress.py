...
def calc_stress(pos_optimized, cell_strain, method_stress_statistics, stress_set_dict, num_last_samples, up, cwd, parameters):
    ...
    if method_stress_statistics == 'dynamic':
        ...
        #vasp.write_vasp('POSCAR', pos_strain, vasp5=True, sort=True, direct=True)
        pos_strain.write("geometry.in", format="aims", scaled=True)
        calculator = Aims("",directory="./")
        calculator.template.write_input(calculator.profile, calculator.directory, pos_strain, parameters, ["energy"])
    ...
    pos_strain_optimized = aims_run(" ".join(indict['parallel_submit_command']))
